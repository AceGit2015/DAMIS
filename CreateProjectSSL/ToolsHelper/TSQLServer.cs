...
namespace ToolsHelper
{
    /// <summary>
    /// 数据访问抽象基础类
    /// Copyright (C) 2012-2013 By liangjw 
    /// 数据访问基础类(基于SqlServer)
    /// </summary>
    public abstract class TSQLServer
    {
        ...
        /// 每页显示的数量
        /// </summary>
        public static readonly string PageSize = System.Configuration.ConfigurationManager.AppSettings["DefaultPageSize"].ToString();
        #endregion
        ...
        public static int GetMinID(string FieldName, string TableName)
        {
            ...
            object obj = TSQLServer.GetSingle(strsql);
            if (obj == null)
            {
                return 0;
            }
        ...
        {
            object obj = TSQLServer.GetSingle(strSql, cmdParms);
            int cmdresult;
            if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
            {
                cmdresult = 0;
        ...
        /// <param name="SqlCmdSqlList">Sql命令行列表</param>
        /// <returns>执行结果 0-由于SQL造成事务失败 -1 由于Sql造成事务失败 1-整体事务执行成功</returns>
        public static int ExecuteSqlTran(List<CommandInfo> list, List<CommandInfo> SqlCmdSqlList)
        {
        ...
        /// <param name="SQLString">查询语句</param>
        /// <returns>DataSet</returns>
        public static DataTable ExecDt(string SQLString)
        {
        ...
        /// <param name="SQLString">查询语句</param>
        /// <returns>DataSet</returns>
        public static DataRow ExecDr(string SQLString)
        {
        ...
        ///// <param name="SQLString">SQL语句</param>
        ///// <returns>DataSet</returns>
        //public static DataTable Query(SqlConnection connection, SqlTransaction trans, string SQLString)
        //{
        //    DataSet ds = new DataSet();
        //    try
        //    {
        ...
        #region 执行执行添加、修改、删除sql事务（触发器）
        /// <summary>
        /// 执行执行添加、修改、删除sql事务(触发器)
        /// </summary>
        /// <param name="procFormName">单据存储过程名称</param>
        /// <param name="procDetailsName">单据明细存储过程名称</param>
        /// <param name="sqlpars">参数数组</param>
        /// <param name="list">泛型集合参数数组</param>
        /// <returns></returns>
        public static int execTrigger(string SQLString)
        {
            int rows = 0;
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                connection.Open();
                //启动数据库事务
                SqlTransaction trigger = connection.BeginTransaction();
                //2.建立cmd对象
                SqlCommand cmd = new SqlCommand(SQLString, connection);
                cmd.Transaction = trigger;
        ...
        #region 批量执行执行添加、修改、删除sql事务（触发器）
        /// <summary>
        /// 批量执行执行添加、修改、删除sql事务(触发器)
        /// </summary>
        /// <param name="procFormName">单据存储过程名称</param>
        /// <param name="procDetailsName">单据明细存储过程名称</param>
        /// <param name="sqlpars">参数数组</param>
        /// <param name="list">泛型集合参数数组</param>
        /// <returns></returns>
        public static Boolean execTriggerAll(ArrayList SQLStringList)
        {
            Boolean flag = false;
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
