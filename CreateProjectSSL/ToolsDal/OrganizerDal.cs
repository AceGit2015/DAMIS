...
namespace ToolsDal
{
    public class OrganizerDal
    {
        ...
        /// 分页获取数据列表（承办单位表）
        /// </summary>
        public PeterPages GetSearch(Organizer Organizer, int PageIndex, int PageSize)
        {
            PeterPages fy = null;
            string sqlwhere = " 1=1 ";

            if (!string.IsNullOrEmpty(Organizer.OrganizerName))
            {
                sqlwhere = sqlwhere + " and OrganizerName like '%" + Organizer.OrganizerName + "%' ";
            }
            PageInfoNew entity = new PageInfoNew();
            entity.Sqlwhere = sqlwhere.Trim();
            entity.Tablename = "[Organizer]";  //用户表，注意如果是多表可以写成视图进行查询，这里就为视图名称
        ...
        {
            return TSQLServer.ExecDt("select * from [Organizer] where 1=1 " + strwhere);
        }
        #endregion
        ...
        {
            return TSQLServer.ExecDr("select * from [Organizer] where id =" + values[0] + "");
        }
        #endregion
        ...
        {
            return TSQLServer.ExecuteNonQuery("delete [Organizer] where id =" + values[0] + "");
        }
        #endregion
        ...
        {
            return TSQLServer.ExecuteNonQuery("delete [Organizer] where id in(" + values + ")");
        }
        #endregion
        ...
        public int Insert(params object[] values)
        {
            ...
            strSql.Append("insert into Organizer(");
            strSql.Append("OrganizerName,UserID,Username,OperateTime)");
            strSql.Append(" values (");
            strSql.Append("@OrganizerName,@UserID,@Username,@OperateTime)");
            strSql.Append(";select @@IDENTITY");
            SqlParameter[] parameters = {
					new SqlParameter("@OrganizerName", SqlDbType.NVarChar,50),
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Organizer set ");
