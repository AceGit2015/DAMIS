...
namespace ToolsDal
{
    public class PunishTypeDal
    {
        #region 搜索列表分页获取数据列表
        /// <summary>
        /// 分页获取数据列表（行政处罚类型表）
        /// </summary>
        public PeterPages GetSearch(PunishType PunishType, int PageIndex, int PageSize)
        {
            PeterPages fy = null;
            string sqlwhere = " 1=1 ";

            if (!string.IsNullOrEmpty(PunishType.PunishTypeName))
            {
                sqlwhere = sqlwhere + " and PunishTypeName like '%" + PunishType.PunishTypeName + "%' ";
            }
            PageInfoNew entity = new PageInfoNew();
            entity.Sqlwhere = sqlwhere.Trim();
            entity.Tablename = "[PunishType]";  //用户表，注意如果是多表可以写成视图进行查询，这里就为视图名称
        ...
        {
            return TSQLServer.ExecDt("select * from [PunishType] where 1=1 " + strwhere);
        }
        #endregion
        ...
        {
            return TSQLServer.ExecDr("select * from [PunishType] where id =" + values[0] + "");
        }
        #endregion
        ...
        {
            return TSQLServer.ExecuteNonQuery("delete [PunishType] where id =" + values[0] + "");
        }
        #endregion
        ...
        {
            return TSQLServer.ExecuteNonQuery("delete [PunishType] where id in(" + values + ")");
        }
        #endregion
        ...
        public int Insert(params object[] values)
        {
            ...
            strSql.Append("insert into PunishType(");
            strSql.Append("PunishTypeName,FileClassName,FileClassID,UserID,Username,OperateTime)");
            strSql.Append(" values (");
            strSql.Append("@PunishTypeName,@FileClassName,@FileClassID,@UserID,@Username,@OperateTime)");
            strSql.Append(";select @@IDENTITY");
            SqlParameter[] parameters = {
					new SqlParameter("@PunishTypeName", SqlDbType.NVarChar,50),
					new SqlParameter("@FileClassName", SqlDbType.VarChar,50),
        ...
        public int Update(params object[] values)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update PunishType set ");
            strSql.Append("PunishTypeName=@PunishTypeName,");
